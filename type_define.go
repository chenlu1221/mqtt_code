...
func (q QoS) IsReturnCode() bool {
	return q < qosFirstInvalid || q == qosFailure
}
...
func writeMessage(w io.Writer, hdr *FixedHeader, payloadBuf *bytes.Buffer, extraLength int32) error {
	...
	//编码固定头
	err := hdr.encodeInto(buf, hdr.MessageType, int32(totalPayloadLength))
	if err != nil {
		return err
	}
...
func encodePubCommon(w io.Writer, hdr *FixedHeader, packetIdentifier uint16) error {
	...
	return writeMessage(w, hdr, buf, 0)
}
